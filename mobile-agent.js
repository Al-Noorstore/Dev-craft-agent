#!/usr/bin/env node
// ============================================
// DEV CRAFT AGENT - MOBILE AGENT (Termux / PC / Mac / Linux)
...
if (!CODE) {
  console.log('\n❌ Pairing code do! Usage: node mobile-agent.js <PAIRING-CODE>');
  console.log('   Website kholo → Connect PC ya Terminal → "Get pairing code" dabao.\n');
  process.exit(1);
}

const isAndroid = process.platform === 'android';
const DEVICE_NAME = isAndroid
  ? '\u{1F4F1} ' + os.hostname() + ' (Android Termux)'
  : os.hostname() + ' (' + (isWin ? 'Windows' : isMac ? 'Mac' : 'Linux') + ')';
